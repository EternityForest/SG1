...
void RFM69::urandom(uint8_t *target, uint8_t len)
{
  ...
  //We just use the encrypted keystream XORed with the stackGarbage
  ...
  while (len)
  {
    cipherContext.encrypt(target, stackGarbage, 1);
    len -= 1;
    target += 1;
  }
...
retry:
  ...
  memcpy(globalScratchpad, (uint8_t *)(&(defaultChannel.privateHintSequence)), 3);

  doTimestamp();

  //Copy all 8 of the IV bytes, but we only send 4
  memcpy(globalScratchpad + 3, systemTimeBytes, 8);

  //Replace the LSB byte with the node ID.
  globalScratchpad[3] = nodeID;

  cipherContext.setKey(defaultChannel.channelKey, 32);
  cipherContext.setIV(globalScratchpad + 3, 8);
  ...
  cipherContext.encrypt(globalScratchpad + 3 + 4, (uint8_t *)buffer, bufferSize);
  cipherContext.computeTag(globalScratchpad + 3 + 4 + bufferSize, 4);
  ...
  if (trySend(globalScratchpad, bufferSize + 3 + 4 + 4))
  {
    debug("srt");
  }
  else
...
retry:
  ...
  memcpy(globalScratchpad, (uint8_t *)(&(defaultChannel.fixedHintSequence)), 3);
  ...
  memcpy(globalScratchpad + 3 + 1, systemTimeBytes + 1, 7);
  ...
  if (useChallenge == 0)
  {
    memcpy(awaitReplyToIv, globalScratchpad + 3, 8);
  }
  ...
  cipherContext.encrypt((uint8_t *)globalScratchpad + 3 + 8, (uint8_t *)buffer, bufferSize - padding);
  ...
  if (padding > 0)
  {
    cipherContext.encrypt((uint8_t *)globalScratchpad + 3 + 8 + bufferSize - padding, &padByte, 1);
  }
  if (padding == 2)
  {
    cipherContext.encrypt((uint8_t *)globalScratchpad + 3 + 8 + (bufferSize - padding) + 1,
                          &padByte, 1);
  }

  cipherContext.computeTag(globalScratchpad + 3 + 8 + bufferSize, 8);
  ...
  if (useFEC)
  {
    ...
    {
      golay_block_encode(globalScratchpad + i, globalScratchpad + 6 + (i * 2));
    }
  }
  else
  {
    //No FEC, just move into place.
    memmove(globalScratchpad + 6, globalScratchpad, payloadSize);
  }

  golay_block_encode(header, globalScratchpad);
  ...
  while ((millis() - st) < 3)
  {
    ...
    if (trySend(globalScratchpad + 1, header[0] - 1))
    {
      debug("snt");
      debug(header[0] - 1);
...
void RFM69::doPerPacketTimeFunctions(uint8_t rxTimeTrust)
{
  ...
  else if ((rxTimeTrust >= systemTimeTrust) && (rxTimeTrust >= TIMETRUST_CHALLENGERESPONSE))
  {
    ...
    rawSendSG1(globalScratchpad, 0, 0, HEADER_TYPE_SPECIAL);
  }
  else
  {
    debug("nj");
...
bool RFM69::decodeSG1Header()
{
  ...
  scratchpad[0] = DATALEN + 1;
  //Copy evrything in the header but the length nyte we already have
  memcpy(scratchpad + 1, DATA, 5);

  if (golay_block_decode(scratchpad, rxHeader))
  {
    debug("badheader");
    return false;
  }
...
void RFM69::loadConnectionFromEEPROM(uint16_t eepromAddr)
{
  ...
  {
    scratchpad[i] = EEPROM.read(eepromAddr + i);
  }
  if (scratchpad[0] == 'S' && scratchpad[1] == 'G')
  {
    defaultChannel.setChannelKey(scratchpad + 2);
    setProfile(scratchpad[2 + 32]);
    setChannelNumber(readUInt16(scratchpad + 2 + 32 + 1));
    setNodeID(scratchpad[2 + 32 + 1 + 2]);
    //addEntropy(scratchpad + 2+ 32 + 1 + 2 + 1, 8);
  }
#endif
}
...
{
  //Do not call from within anything that uses the global scratchpad
    ...
    if (receiveDone())
    {
      ...
      if (state == 0)
      {
        if (DATA[0] == SPECIAL_TYPE_PAIRING_REQUEST)
        {
          ...
          debug("PR");
          //We need random data
          urandom(globalScratchpad, 32);
          Curve25519::dh1(buf + 1, secret, globalScratchpad);
  ...
  while ((millis() - now) < 10000)
  {
    ...
    if (receiveDone())
    {
      ...
      if (DATA[0] == SPECIAL_TYPE_PAIRING_CHALLENGE)
      {
        ...
        //preserve the pubkey
        memcpy(globalScratchpad, buf + 1, 32);
        ...
        memcpy(buf + 1, globalScratchpad, 32);
        ...
        cipherContext.setKey(secret, 32);
        memset(globalScratchpad, 0, 8);
        //IV of 0, because key always changes
        cipherContext.setIV(globalScratchpad, 8);
...
void RFM69::sendTimeReply()
{
  if ((approxUnixMillis() - lastSentTimeAutoreply) > 100)
  {
    ...
    rawSendSG1(globalScratchpad, 0, rxIV, HEADER_TYPE_REPLY_SPECIAL);
...
uint8_t RFM69::decodeSG1()
{
  ...
  if ((rxHeader[1] & HEADER_FEC_FIELD) == HEADER_FEC_GOLAY)
  {
    ...
    for (uint8_t i = 0; i < (tmpDatalen - 5); i += 6)
    {
      ...
      if (golay_block_decode(DATA + i + 5, scratchpad + (i / 2)))
      {
        debug("badfec");
        return 0;
      }
  ...
  else
  {
    ...
    memcpy(scratchpad, DATA + 5, tmpDatalen);
  }

  uint32_t rxHintSequence = readHintSequence(scratchpad);
  ...
  if (checkHintSequenceRelevance(rxHintSequence, false))
  {
    ...
    cipherContext.decrypt(scratchpad + 3 + 8, scratchpad + 3 + 8, remainingDataLen - (3 + 8 + 8));

    if (!cipherContext.checkTag(scratchpad + (remainingDataLen - 8), 8))
    {
      debug("rbc");
      return 0;
    }

    memcpy(DATA, scratchpad + 3 + 8, remainingDataLen - (3 + 8 + 8));
...
bool RFM69::decodeSG1RT()
{
  ...
  //cipherContext.setIV(rxIV, 8);
  defaultChannel.setupCipher(rxIV);
  cipherContext.decrypt(scratchpad, DATA + 3 + 4, DATALEN - (3 + 4 + 4));
  ...
  memcpy(DATA, scratchpad, DATALEN - (3 + 4 + 4));
...
void RFM69::sendBeacon(bool wakeUp)
{
  ...
  golay_block_encode(header, scratchpad);
  if (wakeUp)
  {
    memcpy(scratchpad + 6, &(defaultChannel.privateWakeSequence), 3);
  }
  else
  {
    memcpy(scratchpad + 6, &(defaultChannel.privateHintSequence), 3);
  }
  ...
  if (power > 3)
  {
    memcpy(globalScratchpad, scratchpad + 6, 3);
    golay_block_encode(globalScratchpad, scratchpad + 6);
  }
  ...
  sendFrame(scratchpad + 1, header[0] - 1);
}
...
bool RFM69::isStructured()
{
  if (((rxHeader[1] & HEADER_TYPE_FIELD) == HEADER_TYPE_STRUCTURED))
  {
    return 1;
  }
  return 0;
...
{
  if (((rxHeader[1] & HEADER_TYPE_FIELD) == HEADER_TYPE_RELIABLE_SPECIAL))
  {
    return 1;
  }
  return 0;
...
void RFM69::newStructuredMessage()
{
  txStructuredMessagePointer = 0;
}

uint8_t *RFM69::getStructuredRecord()
{
  if (rxStructuredMessagePointer >= rxStructuredMessageLen)
  {
    return 0;
  }
...
{
  rawSendSG1(txStructuredMessageBuffer, txStructuredMessagePointer, 0, HEADER_TYPE_STRUCTURED);
  txStructuredMessagePointer=0;
}


void RFM69::writeStructuredRecord(uint8_t type, void *data, uint8_t len, uint8_t channel)
{
  uint8_t lenCode = 0;

  //Create the length codel
  if (len == 1)
  {
    return;
  }
  //1 is covered by the default of 0
  else if (len == 2)
  {
    lenCode = 1;
  }
  ...
  //Autoflush instead of buffer overflow
  if (txStructuredMessagePointer + len + 2 > 12)
  {
    flushStructuredMessage();
  }

  txStructuredMessageBuffer[txStructuredMessagePointer]  = type;
  txStructuredMessagePointer++;

  txStructuredMessageBuffer[txStructuredMessagePointer] = (channel << 2) | lenCode;
  txStructuredMessagePointer++;

  memcpy(txStructuredMessageBuffer + txStructuredMessagePointer, data, len);
...
{
  configDataEEPROM = eepromAddr;
  configDataSize = dataSize;

  configData = (uint8_t *)malloc(dataSize);

#if defined(E2END) && E2END > 0
  for (int i = 0; i < dataSize; i++)
  {
    configData[i] = EEPROM.read(eepromAddr + i);
  }
#endif
}

void RFM69::saveConfigData()
{
  if (configData == 0)
  {
    return;
